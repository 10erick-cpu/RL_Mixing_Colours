...
def initialize_agent(experiment_name, agent_name, agent_run_id=0, record_episodes=False):
    ...
    environment = agent_params.build_env(init_helper)

    if record_episodes:
        environment = CustomMonitor(env=environment,
                                    data=DotDict({'cfg': agent_params}),
                                    log_dir=Folder("recorded_episodes", create=True),
                                    log_f_name="play_agent",
                                    print_interval=0,
                                    eval_interval=0)

    agent = agent_params.get_model(environment)
...
if __name__ == '__main__':
    ...
    agent = initialize_agent("example_training", "example_agent", agent_run_id=0, record_episodes=True)
