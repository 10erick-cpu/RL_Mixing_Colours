...
from evaluation.coco_utils import get_coco_api_from_dataset
from evaluation.coco_eval import CocoEvaluator
from model_setup import coco_utils
...
@torch.no_grad()
def evaluate(model, data_loader, device, coco_index=None, iou_types=['bbox', 'segm']):
    ...
    coco = get_coco_api_from_dataset(data_loader) if coco_index is None else coco_index
    #iou_types = _get_iou_types(model)
    coco_evaluator = CocoEvaluator(coco, iou_types)

    for image, targets in metric_logger.log_every(data_loader, 20, header):
